...
def parse_llm_response(response: str, question: str, triples: bool = False):
    qa_llm = ChatOpenAI(model="gpt-4o-mini", temperature=0, model_kwargs={"response_format": {"type": "json_object"} })
    if triples:
        messages = [
        ("system", SECONDARY_TRIPLES_PARSE_RESPONSE_INSTRUCTION),
        ("user", 'Question: \n{}\n\nModel Response: \n{}\nA model has responded in this way to the given question. Please decompose the reasoning steps of this model into independent atomic facts by extracting triples, and shape your output in a JSON format.\nThis would be an example of knowledge triple: <Paris, capital_of, France>'.format(question, response)),
    ]
    else: 
        messages = [
    ...
    reasoning_list = [str(cot) for cot in response_json["facts"]]
...
SECONDARY_TRIPLES_PARSE_RESPONSE_INSTRUCTION = \
"""
...
Please extract the reasoning steps and final answers from the response and return them in a JSON object with one key:
1. "facts": a list of atomic reasoning steps used to answer the question, in shape of knowledge triples (<subject, predicate, object>)
"""
