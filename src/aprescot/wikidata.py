...
class WikiDataKnowledgeGraph:
    ...
    def __init__(
        ...
        # scorer_model: str = "sentence-transformers/all-mpnet-base-v2",  # swap to SRTK scorer if you have it
        device: Optional[str] = None,   # "cuda" | "mps" | "cpu" (auto-detect)
        user_agent: str = "ApresCoT/1.0 (mailto:moeiiinsh@gmail.com)",
        req_timeout_s: int = 60,
        polite_qps: float = 1.0,        # be gentle with the public endpoint
    ):        
        ...
        self._neighbors_cache: Dict[Tuple[str, str], List[Dict[str, str]]] = {}  # (qid,direction) -> triples
        ...
        *,
        ...
        for hop in range(max_hops):
            ...
            for qid in list(frontier):
                ...
                neigh = self._neighbors(qid, direction="both", per_pred_cap=per_pred_cap, total_cap=total_cap_per_node)
                print("Neighbors found:", neigh)
                for t in neigh:
                    key = (t["s"], t["p"], t["o"])
    ...
    def _neighbors(self, qid: str, direction: str = "both", per_pred_cap: int = 32, total_cap: int = 256) -> List[Dict[str, str]]:
    ...
    # ) -> Dict:
    #     """
    #     1) Entity-link the question to Wikidata QIDs (SRTK linker).
    #     2) Iteratively expand a high-recall subgraph using SRTK retriever
    #        with the pretrained scorer model against the SPARQL endpoint.
    #     3) Optionally fetch human-readable labels for nodes/relations.
    ...
    #           "triples": [
    #              {"s":"Qxx","p":"Pyy","o":"Qzz"},
    #              ...  # literals may appear as {"o": {"type":"literal","value":"..."}}
    #           ],
    #           "labels": { "Qxx": "Label", "Pyy": "prop label", ... }   # if add_labels=True
    #         }
    #     """
    #     seeds = self._srtk_link(question, topk=topk_link)
    #     if not seeds:
    #         print("Error - No seeds found.")
    #         return {"question": question, "seeds": [], "triples": [], "labels": {}}

    #     print("Seeds found:", seeds)
    #     triples = self._srtk_retrieve(
    #         seeds=seeds,
    #         max_hops=max_hops,
    #         beam_size=beam_size,
    #         max_nodes=max_nodes,
    ...
    #     result = {"question": question, "seeds": seeds, "triples": triples}

    #     if add_labels and triples:
    #         qids, pids = self._collect_entity_property_ids(triples)
    #         # Add seeds too, so they are labeled
    #         qids.update(seeds)
    #         labels = self._fetch_labels(qids=list(qids), pids=list(pids))
    #         result["labels"] = labels
    #     else:
    #         result["labels"] = {}
    ...
    # # ---------------------------
    # # SRTK: entity linking + retrieval via CLI
    # # ---------------------------
    # def _srtk_link(self, text: str, topk: int = 5) -> List[str]:
    #     """
    #     Uses SRTK's linker to map a natural-language question to Wikidata QIDs.
    #     We call the CLI to avoid depending on internal Python APIs.
    #     """
    #     # Typical CLI pattern (SRTK docs): srtk link --kg wikidata --text "..." --topk 5 --format json
    #     # Some builds expose `--endpoint`, others use internal cfg for linking; endpoint isn't needed for linking.
    #     cmd = (
    #         f'srtk link '
    ...
    #     # Expecting a JSON list of candidates like [{"id":"Q76","score":...}, ...]
    #     if not data:
    #         return []
    #     if isinstance(data, dict) and "candidates" in data:
    #         return [c.get("id") for c in data.get("candidates", []) if c.get("id")]
    #     if isinstance(data, list):
    #         return [c.get("id") for c in data if isinstance(c, dict) and c.get("id")]
    #     return []
    ...
    # ) -> List[Dict[str, str]]:
    #     """
    #     Uses SRTK's retriever with a pretrained scorer, querying the SPARQL endpoint
    #     to iteratively expand a question-relevant subgraph. Returns triples in ID-space.
    #     """
    #     seed_str = ",".join(seeds)
    #     flags = [
    #         "srtk", "retrieve",
    ...
    #         "--scorer-model-path", self.scorer_model,
    ...
    #         flags += ["--include-qualifiers"]

    #     cmd = " ".join(shlex.quote(x) for x in flags)
    #     data = self._run_cli(cmd)

    #     # Expect structure like: {"triples": [{"s":"Qxx","p":"Pyy","o":"Qzz"}, ...], ...}
    #     # Fall back to a list of triples if that's how the tool prints it.
    #     if not data:
    #         return []
    #     if isinstance(data, dict) and "triples" in data:
    #         return data["triples"]
    #     if isinstance(data, list):
    #         # Already a list of triples
    #         return data
    #     return []

    # # ---------------------------
    # # Label fetching (SPARQL)
    # # ---------------------------
    # def _collect_entity_property_ids(self, triples: List[Dict[str, str]]) -> Tuple[set, set]:
    #     qids, pids = set(), set()
    #     for t in triples:
    #         s = t.get("s")
    #         p = t.get("p")
    #         o = t.get("o")
    #         if isinstance(s, str) and s.startswith("Q"):
    #             qids.add(s)
    #         if isinstance(p, str) and p.startswith("P"):
    #             pids.add(p)
    #         # object can be entity Q… or literal
    #         if isinstance(o, str) and o.startswith("Q"):
    #             qids.add(o)
    #     return qids, pids

    # def _fetch_labels(self, qids: List[str], pids: List[str]) -> Dict[str, str]:
    #     """
    #     Fetch English labels for entities (Q*) and properties (P*) via SPARQL.
    #     """
    #     labels = {}
    #     # Chunk to avoid very long queries
    #     CHUNK = 150
    ...
    #     # Entities
    #     for part in chunks(qids, CHUNK):
    #         if not part:
    #             continue
    #         q = self._label_query_for_entities(part)
    #         resp = requests.get(self.endpoint, params={"query": q}, headers=self.headers, timeout=self.timeout_s)
    #         resp.raise_for_status()
    #         for b in resp.json().get("results", {}).get("bindings", []):
    #             qid = b["item"]["value"].rsplit("/", 1)[-1]
    #             lab = b.get("itemLabel", {}).get("value")
    ...
    #     # Properties
    #     for part in chunks(pids, CHUNK):
    #         if not part:
    #             continue
    #         q = self._label_query_for_properties(part)
    #         resp = requests.get(self.endpoint, params={"query": q}, headers=self.headers, timeout=self.timeout_s)
    #         resp.raise_for_status()
    #         for b in resp.json().get("results", {}).get("bindings", []):
    #             pid = b["prop"]["value"].rsplit("/", 1)[-1]
    ...
    # @staticmethod
    # def _label_query_for_entities(qids: List[str]) -> str:
    ...
    # @staticmethod
    # def _label_query_for_properties(pids: List[str]) -> str:
    #     values = " ".join(f"(wd:{pid})" for pid in pids)
    #     return f"""
    #     SELECT ?prop ?propLabel WHERE {{
    #       VALUES (?prop) {{ {values} }}
    #       SERVICE wikibase:label {{ bd:serviceParam wikibase:language "en". }}
    ...
    #     """Run a CLI command and parse JSON output (returns None on failure)."""
    #     try:
    #         proc = subprocess.run(
    ...
    #         print(f"[SRTK CLI error] {err}")
    #         return None
    #     except json.JSONDecodeError:
    #         print("[SRTK CLI] Output was not valid JSON.")
    #         return None
